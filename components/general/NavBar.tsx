    ...
    <div className="w-full flex justify-between items-center md:items-start
      ...
      <div className="flex-initial w-[full] md:w-[auto] mx-[25px] md:mr-[0px] ml-[50px] justify-between md:justify-start md:flex-col text-xl relative font-sans z-99">
        <div className='flex justify-between md:hidden'>
          ...
          <Link href="/login"><div className="text-[white] rounded rounded-[28px] mr-2 py-[4px] px-[18px] border-2 border-transparent bg-[#533483] hover:bg-[#0F3460] cursor-pointer">Login</div></Link>
        </div>
        <div className={`hidden md:flex md:flex-col md:items-end ${isOpen && 'bg-gray-400 bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-40'} ${isOpen && 'w-[200px]'} ${isOpen && 'min-h-[100vh]'} ${isOpen && 'justify-between'} pr-[20px] absolute right-[-30px] top-[-0px] z-[99]`}>
          ...
          {isOpen && 
          <Link href="/login">
            <div className="text-[white] rounded rounded-[16px] py-[4px] px-[20px] bg-[#0F3460] hover:bg-[#7FBCD2] mb-2 mt-20 mx-8 cursor-pointer"
              onClick={
                () => { setOpen(false) }
              }
            >
